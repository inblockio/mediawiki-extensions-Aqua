...
CREATE TABLE IF NOT EXISTS `page_verification` (
	...
	`genesis_hash` VARCHAR(128) COMMENT 'Alternative unique identifier for a page representing the verification_hash of the first revision',
	...
	`previous_verification_hash` VARCHAR(128) COMMENT 'Shows the previous verification_hash for the ealier revision (not applicable on genesis)',
	`verification_hash` VARCHAR(128) COMMENT 'Combined metadata and content hash',
	`next_verification_hash` VARCHAR(128) COMMENT 'Shows the subsequent verification_hash in the chain',
	`seed` VARCHAR(128) COMMENT 'Random SHA3-512 hash for additional entropy on genesis',
